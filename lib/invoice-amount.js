...
// Reads a PDF with BOTH readers and keeps the better answer.
//
// The two lay text out differently, and the archive has files that only one
// of them reads right: pdf-parse fails on a broken cross-reference table
// (Apollo) while pdfjs's line layout defeated the patterns on Bubble's
// invoices and lost three of the four invoices stapled into Anthropic's July
// file — a run that trusted pdfjs alone reported ten Bubble invoices as
// "no total" and lowered Anthropic's July from 13,895.83 to 138.82. So each
// file is read twice and the answers compared:
//   · a usable total beats none;
//   · a total built from more invoices in the file beats one from fewer;
//   · otherwise pdf-parse's answer stands, being the one this archive was
//     reconciled with, and pdfjs's is noted beside it when the two differ.
// The text returned is the chosen reader's; the other's is kept for whoever
// wants to look (?mode=inspect shows both).
async function readInvoiceTotal(buffer, options) {
  const opts = options || {};
  const attempts = [];
  for (const [reader, fn] of [['pdf-parse', readWithPdfParse], ['pdfjs', readWithPdfjs]]) {
    try {
      const { text, pages } = await fn(buffer);
      if (!String(text).trim()) { attempts.push({ reader, error: 'no text (a scanned image?)' }); continue; }
      const total = extractInvoiceTotal(text, { currency: opts.currency, pages });
      attempts.push({ reader, text, pages, total });
    } catch (e) {
      attempts.push({ reader, error: e.message });
    }
  }
  const read = attempts.filter(a => !a.error);
  if (!read.length) {
    return { text: '', pages: null, reader: null, error: `Could not read the PDF (${attempts.map(a => `${a.reader}: ${a.error}`).join('; ')}).`, total: null, attempts };
  }
  const score = a => (a.total.usable ? 2 : a.total.amount !== null ? 1 : 0) * 100 + (a.total.parts ? a.total.parts.length : 1);
  read.sort((x, y) => score(y) - score(x) || (x.reader === 'pdf-parse' ? -1 : 1));
  const best = read[0];
  const other = read.find(a => a !== best);
  const total = { ...best.total };
  if (other && other.total.usable && best.total.usable && Math.abs((other.total.amount || 0) - (best.total.amount || 0)) >= 0.005) {
    total.note = `${other.reader} read ${other.total.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })} instead; ${best.reader}'s figure was kept.`;
    total.disagreed = other.total.amount;
  }
  return { text: best.text, pages: best.pages || null, reader: best.reader, error: null, total, attempts };
}

module.exports = { extractInvoiceTotal, extractInvoiceRef, extractInvoiceRefs, detectCurrency, readPdfText, readInvoiceTotal, linesFromTextContent, TOTAL_PATTERNS, MAX_PLAUSIBLE };
