...
// One PDF, several invoices. Anthropic's "July 26.pdf" is four invoices
// stapled together — three credit top-ups and the month's usage invoice —
// and reading it as one document takes the first Total (138.82) for a file
// that comes to 13,895.83. When the text carries more than one invoice
// number, each invoice's section is totalled on its own and the sections are
// added up.
function compoundTotal(t, opts) {
  const refs = extractInvoiceRefs(t);
  if (refs.length < 2) return null;
  const sections = t.split(/(?=\binvoice\s*(?:number|no\.?|num\.?|#)\b)/i).filter(x => /\binvoice\s*(?:number|no\.?|num\.?|#)/i.test(x));
  if (sections.length < 2) return null;
  const parts = [];
  for (const section of sections) {
    const r = extractInvoiceTotal(section, { ...(opts || {}), single: true });
    ...
    parts.push({ ref: extractInvoiceRef(section), amount: r.amount });
  }
  const total = Math.round(parts.reduce((a, b) => a + b.amount, 0) * 100) / 100;
  return { amount: total, parts, refs };
}
...
function extractInvoiceTotal(text, options) {
  ...
  if (!opts.single) {
    ...
    if (compound) {
      return {
        amount: compound.amount, currency: 'USD', via: `${compound.parts.length} invoices in one file`, usable: true, note: '',
