...
const DATE = [
  ...
  String.raw`[A-Za-z]{3,9}\.?\s+\d{1,2}(?:st|nd|rd|th)?\s*,?\s*\d{4}`,        // Aug 26, 2026; AWS writes "August 31 , 2026"
...
const PERIOD_RE = new RegExp(
  ...
  + String.raw`(?:period|term)s?`
  // Google's invoices and statements head their charges "Summary for Apr 1,
  // 2026 - Apr 30, 2026": the same thing, under another name.
  + String.raw`|summary\s+for)\b\s*[:\-–—]?\s*(?:is\s+|of\s+|for\s+|covering\s+)?(?:from\s+|starting\s+)?`
  + `(${DATE})`
...
// A vendor's line items are its billing period when nothing else states one.
// Anthropic's invoice reads "Claude Opus 4.8 Usage Aug 1 – Aug 31, 2026" on
// every line, Render's "Render Aug 1, 2026 - Aug 31, 2026", dbt's "Seats -
// Developer License Sep 2 – Sep 30, 2026", Apollo's "Basic Seat Sep 2, 2026 –
// Sep 2, 2027" — and none of them says "period" anywhere. Filing those by the
// date of issue put August's usage under September, which the owner called
// completely wrong: the invoice is for the month its first range starts in.
//
// The first range in the text decides, as with labelled periods: dbt bills
// last month's usage and next month's seats on one invoice, and the usage
// line comes first. Anything a month name does not open is not a date here,
// so "Suite 500 - 1" and a zip code never read as a range.
const RANGE_RE = new RegExp(
  `(${DATE})` + String.raw`\s*(?:-|–|—|to|through|thru|until|till)\s*` + `(${DATE})`,
  'gi'
);
// The dash between the two dates is often lost in the extracted text ("Aug 1
// Aug 31, 2026"), so a month-word date without a year followed by one with a
// year is read as a range too. Only that form: two full dates side by side
// ("September 1, 2026 October 1, 2026") are how a PDF's date-of-issue and
// due-date columns come out, and reading those as a period would file
// August's usage under September again.
const MONTH_WORD = String.raw`(?:jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\.?`;
const RANGE_SPACE_RE = new RegExp(
  `(${MONTH_WORD}\\s+\\d{1,2}(?:st|nd|rd|th)?)\\s+(${MONTH_WORD}\\s+\\d{1,2}(?:st|nd|rd|th)?\\s*,?\\s*\\d{4})`,
  'gi'
);
// Kept for callers that still name it: Anthropic's rule opted in to usage
// ranges when only it needed them. Every vendor gets them now.
const USAGE_RANGE_RE = RANGE_RE;
...
function extractInvoiceDate(text) {
  ...
  while ((m = INVOICE_DATE_RE.exec(flat)) !== null) {
    ...
    // "Date due  Date of issue  September 9, 2026  September 2, 2026": the
    // date after "issue" is the due date's, because the labels are a block
    // with their values after it. A label preceded by another label is left
    // to the pairing below.
    if (/\b(date\s+of\s+issue|invoice\s+date|issue\s+date|date\s+paid|payment\s+date|due\s+date|date\s+due|invoice\s+number|receipt\s+number)\s*[:\-]?\s*\w*\s*$/i.test(flat.slice(Math.max(0, m.index - 24), m.index))) continue;
  ...
  return pairedInvoiceDate(flat);
}

// The same, for a PDF whose header is two columns that come out as one column
// after the other: Stripe's "Invoice number  Date of issue  Date due
// Q8MUNTUC-0204  September 2, 2026  September 2, 2026", or Google's values
// before their labels ("5571764174  Apr 30, 2026  7476-5812-7596  …  Invoice
// number  Invoice date  Billing ID"). No date sits next to its label, so the
// labels and the values are paired by order instead: the k-th date-shaped
// label goes with the k-th date in the run of values beside the label block.
const DATE_LABEL_RE = /\b(date\s+of\s+issue|invoice\s+date|issue\s+date|date\s+paid|payment\s+date|due\s+date|date\s+due|invoice\s+number|receipt\s+number|billing\s+id|domain\s+name)\b/gi;
// Only dates that carry a year can be paired, and a token must start on its
// own: "5571764174  Apr 30, 2026" must not yield "74 Apr 30".
const DATE_WITH_YEAR = [
  String.raw`\d{4}[-/.]\d{1,2}[-/.]\d{1,2}`,
  String.raw`[A-Za-z]{3,9}\.?\s+\d{1,2}(?:st|nd|rd|th)?,?\s*\d{4}`,
  String.raw`\d{1,2}[-/.\s][A-Za-z]{3,9}[-/.\s,]+\d{4}`,
  String.raw`\d{1,2}[-/.]\d{1,2}[-/.]\d{2,4}`,
].join('|');
const DATE_TOKEN_RE = new RegExp(String.raw`(?<![\w-])(${DATE_WITH_YEAR})(?![\w])`, 'g');
const ISSUE_LABELS = /^(date\s+of\s+issue|invoice\s+date|issue\s+date|date\s+paid|payment\s+date)$/i;
const DATE_LABELS = /^(date\s+of\s+issue|invoice\s+date|issue\s+date|date\s+paid|payment\s+date|due\s+date|date\s+due)$/i;

function pairedInvoiceDate(flat) {
  const labels = [];
  DATE_LABEL_RE.lastIndex = 0;
  let m;
  while ((m = DATE_LABEL_RE.exec(flat)) !== null) labels.push({ text: m[1].toLowerCase().replace(/\s+/g, ' '), at: m.index, end: m.index + m[0].length });
  const issueIdx = labels.findIndex(l => ISSUE_LABELS.test(l.text));
  if (issueIdx === -1) return null;
  // The block of labels the issue label sits in: neighbours within a short gap.
  let lo = issueIdx, hi = issueIdx;
  while (lo > 0 && labels[lo].at - labels[lo - 1].end < 12) lo--;
  while (hi < labels.length - 1 && labels[hi + 1].at - labels[hi].end < 12) hi++;
  const block = labels.slice(lo, hi + 1);
  const dateLabels = block.filter(l => DATE_LABELS.test(l.text));
  const k = dateLabels.findIndex(l => ISSUE_LABELS.test(l.text));
  if (k === -1) return null;

  const dates = [];
  DATE_TOKEN_RE.lastIndex = 0;
  while ((m = DATE_TOKEN_RE.exec(flat)) !== null) dates.push({ raw: m[1], at: m.index, end: m.index + m[0].length });
  const blockStart = block[0].at, blockEnd = block[block.length - 1].end;
  const WINDOW = 160;
  const after = dates.filter(d => d.at >= blockEnd && d.at - blockEnd < WINDOW);
  const before = dates.filter(d => d.end <= blockStart && blockStart - d.end < WINDOW);
  // Values follow the labels (Stripe) or precede them (Google); take whichever
  // side has enough dates to pair with, preferring the one after.
  const run = after.length >= dateLabels.length ? after : (before.length >= dateLabels.length ? before : null);
  if (!run) return null;
  const chosen = run[k];
  const dayFirst = /^\d{1,2}[-/.]\d{1,2}/.test(chosen.raw) && +chosen.raw.split(/[-/.]/)[0] > 12;
  const d = parseDate(chosen.raw, dayFirst);
  if (!d || d.year === null || !valid(d)) return null;
  return { date: iso(d), month: ym(d), text: `${dateLabels[k].text} → ${chosen.raw}`, paired: true };
}
...
function extractBillingPeriod(text) {
  const flat = String(text || '').replace(/\s+/g, ' ');
  // A labelled period first, wherever it sits; then the first line-item range.
  for (const re of [PERIOD_RE, RANGE_RE, RANGE_SPACE_RE]) {
    const found = firstRange(flat, re);
    if (found) return found;
  }
  return null;
}

function firstRange(flat, re) {
  re.lastIndex = 0;
  let m;
...
// The month before "2026-09" is "2026-08". For a vendor that bills in arrears
// and states no period: its invoice dated in a month is the previous month's.
...
module.exports = { extractBillingPeriod, extractInvoiceDate, monthOfPeriod, monthForPeriod, invoiceMonth, previousMonth, parseDate, PERIOD_RE, RANGE_RE, RANGE_SPACE_RE, USAGE_RANGE_RE };
