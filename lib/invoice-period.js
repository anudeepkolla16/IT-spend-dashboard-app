...
// days of that fall in August and twenty-six in September, and the charge itself
...
function invoiceMonth(text, receivedMonth) {
  ...
  const resolved = monthOfPeriod(period);
  if (!resolved) return { month: receivedMonth, via: 'received', period };

  // A period more than two months from the mail is far more likely to be a
  // misread than a real cycle — a contract term, or a renewal date pair. The
  // mail date is the safer answer there.
  const gap = receivedMonth ? monthsApart(resolved.month, receivedMonth) : 0;
  if (receivedMonth && (gap === null || gap > 2)) {
    return { month: receivedMonth, via: 'received', period, ignoredPeriod: true };
  }

  return { month: resolved.month, via: resolved.rule, period };
}

module.exports = { extractBillingPeriod, monthOfPeriod, invoiceMonth, parseDate, PERIOD_RE };
