...
// A metered vendor's line items ARE its billing period — Anthropic's invoice
// reads "Claude Opus 4.8 Usage Jul 2 – Jul 31, 2026" on every line and states
// no period anywhere else, and under the owner's rule that invoice, issued on
// 1 August, is July's. Line-item ranges are ignored by default (Bubble bills a
// dozen apps on separate cycles inside one month), so a vendor has to opt in
// through its rule: `"period": "usage"`. The dash between the dates is often
// lost in the text, so plain space is accepted as the separator here.
const USAGE_RANGE_RE = new RegExp(
  String.raw`\busage\b\s*[:\-–—]?\s*` + `(${DATE})` + String.raw`\s*(?:-|–|—|to|through|thru|\s)\s*` + `(${DATE})`,
  'gi'
);
...
function extractBillingPeriod(text, options) {
  const flat = String(text || '').replace(/\s+/g, ' ');
  const re = options && options.usageRange ? USAGE_RANGE_RE : PERIOD_RE;
  re.lastIndex = 0;
  let m;
...
function invoiceMonth(text, receivedMonth, options) {
  const period = extractBillingPeriod(text) || (options && options.usageRange ? extractBillingPeriod(text, options) : null);
  if (!period) return { month: receivedMonth, via: 'received', period: null };
  return { ...monthForPeriod(period, receivedMonth), period };
}

module.exports = { extractBillingPeriod, extractInvoiceDate, monthOfPeriod, monthForPeriod, invoiceMonth, parseDate, PERIOD_RE, USAGE_RANGE_RE };
