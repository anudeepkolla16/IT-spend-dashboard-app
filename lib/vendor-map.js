...
const SEED_ALIASES = {
  ...
  granola: ['Granola', 'Granola Business'],
  granolabusiness: ['Granola', 'Granola Business'],
  helpjuice: 'Helpjuice',
  hex: 'Hex',
  hextech: 'Hex',
  hubspot: 'Hubspot',
  ...
  windsurfpro: ['windsurf', 'windsurf pro'],
  xcorp: 'X corp',
...
// Names the invoice checklist leaves alone, matched against archive folders and
// sheet rows alike by the same normalized form. These are not gaps waiting to be
// filled: "Courier bills" and "Laptops sold" are archive folders with no row to
// join to, and "Antivirus,MDM" is a row nobody files invoices for. Counting them
// reports a shortfall that no amount of filing could close, which is what makes
// a real gap easy to miss.
const IGNORED = ['Courier bills', 'Laptops sold', 'Antivirus,MDM'];
const IGNORED_KEYS = new Set(IGNORED.map(norm));
const isIgnored = (name) => IGNORED_KEYS.has(norm(name));
...
const DESCRIPTOR_RULES = [
  ...
  [/windsurf/i, ['windsurf', 'windsurf pro']],
  [/t-?mobile/i, 'TMobile'],
  [/twilio|sendgrid/i, 'Twilo Sendgrid'],
  [/zoho/i, 'ZOHO Books'],
  [/sentry/i, 'Sentry.io'],
  [/granola/i, ['Granola', 'Granola Business']],
...
//
// A target may be several spellings rather than one, first match winning. The
// sheet gets renamed — "Granola Business" became "Granola", "windsurf pro"
// became "windsurf" — and a target naming only the old spelling resolves to
// nothing, so the descriptor rule that depends on it stops matching and the
// charge becomes an exception to map by hand. Listing both spellings keeps the
// rule working across the rename in either direction.
function resolveToSheetApp(candidate, appNames) {
  ...
  for (const c of candidates) {
    if (!c) continue;
    const nc = norm(c);
    const hit = appNames.find(a => norm(a) === nc);
    if (hit) return hit;
  }
  return null;
}
...
module.exports = { norm, bestMatch, buildResolver, resolveToSheetApp, refineAnthropic, SEED_ALIASES, DESCRIPTOR_RULES, IGNORED, isIgnored };
