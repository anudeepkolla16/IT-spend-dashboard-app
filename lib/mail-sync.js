...
function monthForInvoice(text, receivedMonth) {
...
async function runMailSync(token, driveId, options) {
  ...
  const summary = newSummary({ mailbox, since, scanned: messages.length, base });
    ...
    for (const att of pdfs) {
      ...
      try {
        ...
        const when = pdfError
          ...
          : monthForInvoice(pdfText, receivedMonth);
        ...
        const base_meta = {
          ...
          month: when.month, monthVia: when.via,
        ...
        if (!verdict.app || !when.month) {
          const questions = [];
          let reason = verdict.app ? 'no-month' : verdict.reason;
          if (!verdict.app) questions.push(verdict.question);
          if (!when.month) questions.push(`Which month is it for? (${when.why})`);
          if (pdfError) reason = 'unreadable';
          const guess = !verdict.app ? [signals.subject, signals.attachmentNames.join(' '), signals.senderName].map(t => suggest('', t)).find(h => h.app || h.suggestion) : null;
          const { item, already } = await pending.holdInvoice(token, driveId, root, held, bytes, {
            ...base_meta, app: verdict.app || null, reason, question: questions.join(' '),
        ...
        const attApp = verdict.app;
        const attMonth = when.month;
        ...
        if (attMonth !== receivedMonth && !summary.reroutedByPeriod.some(r => r.file === fileName)) {
          ...
          summary.reroutedByPeriod.push({
            file: fileName, app: attApp, from: receivedMonth, to: attMonth, via: when.via,
        ...
        indexEntries.push({
          ...
          monthVia: when.via, periodStart: base_meta.periodStart, periodEnd: base_meta.periodEnd, invoiceDate: base_meta.invoiceDate,
  ...
  await settleMonths(token, driveId, { sheet, INDEX_PATH, deadline }, touched, marks, indexEntries, summary);
...
async function settleMonths(token, driveId, ctx, touched, marks, indexEntries, summary) {
  ...
  // Tick the invoice tracker and set the cells.
            ...
            for (const r of ok) {
              ...
              if (u) {
                summary.amountsUpdated.push({
                  ...
                  previous: u.previous, direction: u.direction,
...
async function resolvePending(token, driveId, answers, options) {
  ...
  if (manualAmounts.length) {
    const index = (await readJsonFile(token, driveId, INDEX_PATH)) || {};
    const amountsList = (Array.isArray(index.amounts) ? index.amounts : []).filter(e => !manualAmounts.some(m => m.path === e.path));
    await writeJsonFile(token, driveId, INDEX_PATH, { ...index, amounts: amountsList.concat(manualAmounts).slice(-2000), updatedAt: new Date().toISOString() });
  }
  ...
  await settleMonths(token, driveId, { sheet, INDEX_PATH, deadline: opts.deadline || (Date.now() + DEADLINE_MS) }, touched, marks, indexEntries, summary);
  summary.amountsUnread = dedupeByFile(summary.amountsUnread);
  summary.amountsNeedingReview = dedupeByFile(summary.amountsNeedingReview);
  summary.pendingCount = held.items.length;
  return { ok: true, ranAt: new Date().toISOString(), ...summary };
...
function formatRunReport(summary, heldItems, extra) {
  ...
  for (const a of s.answered || []) lines.push(`• ${a.id} ${a.file} → ${a.app} ${a.month} (your answer)`);
  ...
  if (wrote.length || upd.length) {
    ...
    for (const u of upd) lines.push(`• ${u.app} ${u.month}: ${money(u.previous)} → ${money(u.amount)}${u.direction === 'down' ? ' (lowered — are all that month\'s invoices in the folder?)' : ''}${u.wasOurs ? '' : ' (replaced a figure not from invoices)'}`);
  }
...
module.exports = { runMailSync, resolvePending, collectSlackAnswers, notify, formatRunReport, readRules, writeRules, monthForInvoice, appToSourceFolder, reconcileArchive, recentMonths, RECONCILE_MONTHS, monthFolderName, planTrackerCells, planAmountCells, folderChildren, sumFolderInvoices };
