...
function planAmountCells(amounts, grid, used, cellValueOf, priorWrites) {
  ...
  for (const [app, byMonth] of Object.entries(amounts || {})) {
    ...
    for (const [month, amount] of Object.entries(byMonth)) {
      ...
      // Invoices for a month arrive across it — Bubble's ninth August invoice
      // lands on the 28th. Without this the first write of the month would stand
      // for ever and the cell would stay short, looking final. So a cell this
      // sync wrote itself, and which still holds exactly what it wrote, may be
      // topped up. Anything else — a human figure or a statement figure — is
      // still never touched.
      const mine = prior[`${app}||${month}`];
      const untouchedSinceOurWrite = mine !== undefined && Math.abs(current - mine) < CELL_EPSILON;

      if (untouchedSinceOurWrite && Math.abs(amount - current) >= CELL_EPSILON) {
        updated.push({ app, month, address, value: amount, previous: current });
        continue;
      }
      if (untouchedSinceOurWrite) continue; // already correct, nothing to do
  ...
  for (const message of messages) {
    ...
    for (const att of pdfs) {
      ...
      try {
        ...
        if (app && month) touched.set(`${app}||${month}`, { app, month, folder: targetFolder });

        if (app && total.amount !== null && !total.usable) {
          ...
          if (toWrite.length) {
            ...
            summary.amountsUpdated = ok.filter(r => planned.updated.some(u => u.address === r.address))
              .map(r => {
                ...
                return { app: r.app, month: r.month, address: r.address, amount: r.value, previous: u && u.previous };
              });
