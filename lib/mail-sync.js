...
const { extractInvoiceTotal, extractInvoiceRef, readPdfText, readInvoiceTotal } = require('./invoice-amount');
const { invoiceMonth, previousMonth, extractInvoiceDate } = require('./invoice-period');
...
// Adds up a month's files invoice by invoice. A month holds single-invoice
// files and merged files side by side, and sometimes the same invoice in
// both — a receipt beside its invoice, a merged file that repeats one already
// filed on its own. Every invoice number is counted once, whichever file it
// is seen in first: a merged file contributes the invoices in it that have
// not been counted yet, never all or nothing. Only a number actually read
// counts as identity; two files that merely share an amount are two charges.
function tallyInvoices(items) {
  let total = 0;
  let counted = 0;
  const unusable = [];
  const unread = [];
  ...
  for (const { name, entry } of items) {
    if (!entry.usable) {
      if (entry.amount !== null) unusable.push({ file: name, amount: entry.amount, currency: entry.currency, note: entry.note });
    ...
    if (Array.isArray(entry.parts) && entry.parts.length) {
      ...
      for (const part of entry.parts) {
        if (part.ref && seenRefs.has(part.ref)) { repeats.push({ file: name, amount: part.amount, ref: part.ref, of: seenRefs.get(part.ref), within: true }); continue; }
        if (part.ref) seenRefs.set(part.ref, name);
        sum += part.amount; fresh++;
      }
      if (fresh) { total += sum; counted++; duplicates.push(...repeats); }
      else duplicates.push({ file: name, amount: entry.amount, ref: entry.parts[0].ref, of: seenRefs.get(entry.parts[0].ref) });
      continue;
    }
    const refs = entry.refs || (entry.ref ? [entry.ref] : []);
    const dup = refs.find(r => seenRefs.has(r));
    if (dup) { duplicates.push({ file: name, amount: entry.amount, ref: dup, of: seenRefs.get(dup) }); continue; }
    for (const r of refs) seenRefs.set(r, name);
    total += entry.amount;
    counted++;
  }
  return { total: Math.round(total * 100) / 100, counted, duplicates, unusable, unread };
}
...
// Bumped whenever the way a PDF is read or totalled changes in a way that can
// alter a figure already on record: every archived file is then read once more
// under the new rules. 2: merged files are split into their invoices by page.
...
async function sumFolderInvoices(token, driveId, folderPath, cache, budget, extraFiles) {
  ...
  for (const { name, key } of entries) {
    ...
    // An entry parsed before the current reading rules is read again: it may
    // hold a truncated invoice number, the first of several totals in a merged
    // file, or no total at all for a file the current reader handles. Once per
    // file — the re-read records the rules it was read under.
    if (entry && entry.parsed !== PARSE_VERSION && !entry.manual) entry = null;
    ...
    if (!entry) {
      ...
      try {
        ...
        const { text, error, reader, total } = await readInvoiceTotal(bytes, budget.options || {});
        entry = error ? { amount: null, currency: null, usable: false, note: error } : total;
      ...
      cache.set(key, entry);
      entry.parsed = PARSE_VERSION;
      budget.fresh.push({
        path: key, amount: entry.amount, currency: entry.currency, usable: !!entry.usable,
        ref: entry.ref || null, refs: entry.refs || null, parts: entry.parts || null, note: entry.note || '', reader: entry.reader, via: entry.via || null, parsed: PARSE_VERSION,
      });
    }

    tallied.push({ name, entry });
  }

  const tally = tallyInvoices(tallied);
  const { total, counted, duplicates, unusable, unread } = tally;
...
function planAmountCells(amounts, grid, used, cellValueOf, priorWrites, partialTotals, locks, allowLower) {
  const rowByApp = new Map(grid.apps.map(a => [a.name, a.rowIdx]));
  const allow = allowLower instanceof Set ? allowLower : (Array.isArray(allowLower) ? new Set(allowLower) : null);
  const lowered = [];
  ...
  for (const [app, byMonth] of Object.entries(amounts || {})) {
    ...
    for (const [month, amount] of Object.entries(byMonth)) {
      ...
      const mine = prior[`${app}||${month}`];
      // A LOWER total is never written on the sync's own judgement. It says
      // one of two opposite things — the archive is missing invoices, or the
      // sheet is stale — and the owner has said, twice, that the first is the
      // usual one (Luzmo's statement figures, Bubble's July). So the cell is
      // held and a question raised; the owner answers keep, invoices, or a
      // figure, and only an answer (or a lock) lowers a cell. The one
      // exception is a month whose invoices this sync moved elsewhere.
      if (amount < current - CELL_EPSILON && !(allow && allow.has(`${app}||${month}`))) {
        lowered.push({ app, month, address, current, invoiceTotal: amount, wasOurs: mine !== undefined && Math.abs(current - mine) < CELL_EPSILON });
        continue;
      }
      updated.push({
  ...
  return { write, updated, skippedFilled, locked, lowered };
}
...
async function readRules(token, driveId, root) {
  ...
  if (raw && Array.isArray(raw.vendors)) {
    // Fill in what the seed has gained since this file was written (never
    // overwriting anything the owner set), and save it if anything was added.
    const up = rulesLib.upgradeRules(raw);
    const rules = rulesLib.normalizeRules(up.rules);
    if (up.changed) await writeJsonFile(token, driveId, path, { ...rules, savedAt: new Date().toISOString(), note: raw.note || 'Edit from the dashboard, or here. See lib/invoices/rules.js for the shape.' });
    return rules;
  }
  const seeded = rulesLib.normalizeRules({ ...rulesLib.SEED_RULES, locks: rulesLib.SEED_LOCKS });
...
// The month an invoice is for. Its billing period decides — the month the
// period starts in — and the first line-item range counts as a period. With no
// period stated at all, the date of issue on its own is not enough: an invoice
// dated 1 September is August's from a vendor that bills usage in arrears
// (AWS, Anthropic, Render) and September's from one that bills the month
// ahead (Zapier). Filing those by their date put August under September,
// which the owner called completely wrong. Which kind the vendor is, is kept
// on its rule (`period: "arrears"` / `"advance"`) from the owner's first
// answer; until then the sync asks. Options: { convention, vendor }.
function monthForInvoice(text, receivedMonth, options) {
  const opts = options || {};
  const placement = invoiceMonth(text, receivedMonth);
  ...
  if (issued && opts.convention === 'advance') {
    return { month: issued.month, via: 'invoice-date (billed in advance)', period: null, issued };
  }
  if (issued && opts.convention === 'arrears') {
    return { month: previousMonth(issued.month), via: 'month before invoice date (billed in arrears)', period: null, issued };
  }
  if (issued) {
    return {
      month: null, via: 'no-period', period: null, issued,
      why: `the invoice states no billing period, only its date (${issued.date}) — is it for that month, or for the month before? The answer is remembered for ${opts.vendor || 'this vendor'}`,
    };
  }
  return { month: null, via: 'unknown', period: null, issued: null, why: 'nothing in the invoice says which month it bills' };
}

// The index's filing log, with one line per file and month: a file re-read,
// re-answered or re-filed replaces its earlier line rather than joining it
// (the log had grown to three lines for one dbt invoice). Latest wins.
function mergeEntries(existing, added) {
  const all = (Array.isArray(existing) ? existing : []).concat(Array.isArray(added) ? added : []);
  const byKey = new Map();
  for (const e of all) {
    if (!e || typeof e !== 'object') continue;
    byKey.delete(`${e.app}||${e.month}||${e.file}`);
    byKey.set(`${e.app}||${e.month}||${e.file}`, e);
  }
...
async function runMailSync(token, driveId, options) {
  ...
  for (const message of messages) {
    ...
    for (const att of pdfs) {
      ...
      try {
        ...
        // The vendor's currency preference is only known once the rules have
        // seen the text, so read first, then total once more with it if needed.
        const first = await readInvoiceTotal(bytes);
        const pdfText = first.text, pdfError = first.error;
        ...
        const total = pdfError
          ...
          : (verdict.currency ? (await readInvoiceTotal(bytes, { currency: verdict.currency })).total : first.total);
        const ref = pdfError ? null : extractInvoiceRef(pdfText);
        const when = pdfError
          ? { month: null, via: 'unreadable', period: null, issued: null, why: pdfError }
          : monthForInvoice(pdfText, receivedMonth, { convention: rulesLib.conventionOf(verdict.period), vendor: verdict.vendor });
  ...
  await settleMonths(token, driveId, { sheet, INDEX_PATH, deadline, locks: rules.locks, held, root }, touched, marks, indexEntries, summary);
...
async function rereadHeld(token, driveId, ctx) {
  ...
  for (const item of candidates) {
    ...
    try {
      ...
      const total = verdict.currency ? (await readInvoiceTotal(bytes, { currency: verdict.currency })).total : first.total;
      const when = monthForInvoice(text, item.receivedMonth, { convention: rulesLib.conventionOf(verdict.period), vendor: verdict.vendor || item.vendor });
      Object.assign(item, {
        amount: total.amount, currency: total.currency, amountUsable: !!total.usable, amountNote: total.note || '',
...
async function settleMonths(token, driveId, ctx, touched, marks, indexEntries, summary) {
  ...
  if (touched.size) {
    ...
    for (const e of (Array.isArray(index.amounts) ? index.amounts : [])) {
      if (e && e.path) cache.set(e.path, { amount: e.amount, currency: e.currency, usable: !!e.usable, note: e.note || '', ref: e.ref || null, refs: e.refs || null, parts: e.parts || null, reader: e.reader, parsed: e.parsed, manual: !!e.manual });
    }
    ...
    if (budget.fresh.length || indexEntries.length) {
      ...
      await writeJsonFile(token, driveId, INDEX_PATH, {
        ...index,
        entries: mergeEntries(index.entries, indexEntries),
    ...
    try {
      ...
      if (hasAmounts) {
        try {
          ...
          const planned = planAmountCells(amounts, live.grid, { values: live.values, start: live.start }, cellValue, priorWrites, partial, ctx.locks, ctx.allowLower);
          ...
          summary.amountsLowered = (summary.amountsLowered || []).concat(planned.lowered);
          // Each held lowering becomes a question the owner can answer from the
          // dashboard or the DM. Raised here, where the figures are known.
          if (planned.lowered.length && ctx.held) {
            let changed = false;
            for (const l of planned.lowered) {
              const money = n => Number(n).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
              const { item, already } = pending.holdCell(ctx.held, {
                app: l.app, month: l.month, sheetValue: l.current, invoiceTotal: l.invoiceTotal, address: l.address,
                question: `The sheet has ${money(l.current)} for ${l.app} ${l.month}; the invoices on file come to ${money(l.invoiceTotal)}. Keep the sheet figure (this locks it), write the invoice total, or set a figure?`,
              });
              if (!already) { changed = true; (summary.heldItems = summary.heldItems || []).push(item); summary.held = (summary.held || 0) + 1; }
            }
            if (changed && ctx.root) await pending.writePending(token, driveId, ctx.root, ctx.held);
          }
  ...
  if (indexEntries.length) {
    const index = (await readJsonFile(token, driveId, INDEX_PATH)) || {};
    const merged = mergeEntries(index.entries, indexEntries);
...
async function resolvePending(token, driveId, answers, options) {
  ...
  for (const ans of Array.isArray(answers) ? answers : []) {
    ...
    try {
      // A question about a cell: keep locks the sheet figure, a figure locks
      // that figure, and "invoices" lets this one lowering through.
      if (item.kind === 'cell') {
        const key = `${item.app}||${item.month}`;
        if (ans.keep || ans.value !== undefined) {
          const value = ans.value !== undefined ? Number(ans.value) : Number(item.sheetValue);
          if (!Number.isFinite(value)) { summary.errors.push(`${item.id}: that is not a figure`); continue; }
          rules.locks = (rules.locks || []).filter(l => !(normName(l.app) === normName(item.app) && l.month === item.month));
          rules.locks.push({ app: item.app, month: item.month, value, note: `Set by the owner on ${new Date().toISOString().slice(0, 10)} (invoices on file came to ${Number(item.invoiceTotal).toLocaleString('en-US', { minimumFractionDigits: 2 })}).` });
          rulesChanged = true;
          summary.learned.push(`${item.app} ${item.month} locked at ${value.toLocaleString('en-US', { minimumFractionDigits: 2 })}`);
        } else if (ans.useInvoices) {
          allowLower.add(key);
        } else if (ans.ignore) {
          // Nothing to do beyond closing the question.
        } else { summary.errors.push(`${item.id}: reply keep, invoices, or a figure`); continue; }
        // Either way the month is totalled again: a lock is enforced, a
        // permitted lowering written.
        const vendorFolder = vendorFolderFor(item.app);
        const place = await placeFor(vendorFolder, item.month);
        if (!touched.has(key)) { touched.set(key, { app: item.app, month: item.month, folder: place.folder }); }
        pending.removeItem(held, item.id);
        stateChanged = true;
        summary.answered.push({ id: item.id, file: item.file, app: item.app, month: item.month, cell: true });
        continue;
      }
      if (ans.ignore) {
        // Ignoring must always close the question. _Ignored may already hold a
        // file of this name from an earlier round (the file takes a numbered
        // name), and the held file may already be gone (nothing to move).
        const dest = `${base}/_Ignored`;
        const moved = await pending.moveHeld(token, driveId, item, dest, { rename: true, tolerateMissing: true });
        pending.removeItem(held, item.id);
        stateChanged = true;
        summary.ignored.push({ id: item.id, file: item.file, ...(moved ? {} : { gone: true }) });
        continue;
      }
      ...
      if (place.files.has(item.file)) {
        ...
        await pending.moveHeld(token, driveId, item, `${base}/_Ignored`, { rename: true, tolerateMissing: true }).catch(() => null);
        summary.alreadyPresent++;
      } else {
      ...
      // A month given for an invoice that stated none says how this vendor
      // bills; the next such invoice from it files itself.
      if (ans.month) {
        const billing = rulesLib.learnPeriod(rules, item, month);
        if (billing.learned) { rules = billing.rules; rulesChanged = true; summary.learned.push(billing.learned); }
      }
  ...
  await settleMonths(token, driveId, { sheet, INDEX_PATH, deadline: opts.deadline || (Date.now() + DEADLINE_MS), locks: rules.locks, held, root, allowLower }, touched, marks, indexEntries, summary);
  summary.amountsUnread = dedupeByFile(summary.amountsUnread);
  summary.amountsNeedingReview = dedupeByFile(summary.amountsNeedingReview);
  summary.pendingCount = held.items.length;
  return { ok: true, ranAt: new Date().toISOString(), ...summary };
...
function formatRunReport(summary, heldItems, extra) {
  ...
  for (const a of s.answered || []) lines.push(a.cell ? `• ${a.id} ${a.app} ${a.month}: answered` : `• ${a.id} ${a.file} → ${a.app} ${a.month}${(s.rereadFiled || []).some(r => r.id === a.id) ? ' (read again and filed by the rules)' : ' (your answer)'}`);
  for (const i of s.ignored || []) lines.push(`• ${i.id} ${i.file} left out, as you said${i.gone ? ' (the held file was already gone)' : ''}`);
  ...
  const low = s.amountsLowered || [];
  if (low.length) {
    lines.push(`Not lowered — the invoices on file come to less than the sheet holds, so I asked instead of writing:`);
    for (const l of low) lines.push(`• ${l.app} ${l.month}: sheet ${money(l.current)}, invoices ${money(l.invoiceTotal)}`);
  }
  ...
  if (rerouted.length) {
    ...
    for (const r of rerouted.slice(0, 10)) {
      const how = r.periodStart ? `period ${r.periodStart} → ${r.periodEnd}` : (r.invoiceDate ? `invoice dated ${r.invoiceDate}, ${/arrears/.test(r.via || '') ? 'billed in arrears' : 'billed in advance'}` : '');
      lines.push(`• ${r.app} ${r.file}: ${r.from} → ${r.to}${how ? ` (${how})` : ''}${r.alsoStillAt ? ` (a copy is still in ${r.alsoStillAt} — delete it or the month counts twice)` : ''}`);
    }
  }
...
module.exports = { runMailSync, resolvePending, mergeEntries, collectSlackAnswers, notify, formatRunReport, readRules, writeRules, monthForInvoice, companionsOf, rereadHeld, tallyInvoices, appToSourceFolder, reconcileArchive, recentMonths, RECONCILE_MONTHS, monthFolderName, planTrackerCells, planAmountCells, folderChildren, sumFolderInvoices };
