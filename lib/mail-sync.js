  ...
  for (const { name, key } of entries) {
    ...
    if (!entry) {
      ...
      budget.fresh.push({
        ...
        ref: entry.ref || null, refs: entry.refs || null, note: entry.note || '', reader: entry.reader, via: entry.via || null, parsed: PARSE_VERSION,
      });
    }
...
async function settleMonths(token, driveId, ctx, touched, marks, indexEntries, summary) {
  ...
  if (touched.size) {
    ...
    for (const e of (Array.isArray(index.amounts) ? index.amounts : [])) {
      if (e && e.path) cache.set(e.path, { amount: e.amount, currency: e.currency, usable: !!e.usable, note: e.note || '', ref: e.ref || null, refs: e.refs || null, reader: e.reader, parsed: e.parsed, manual: !!e.manual });
    }
