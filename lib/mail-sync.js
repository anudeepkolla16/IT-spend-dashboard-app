...
const { monthFromFileName, detectDayFirst } = require('./invoices/inventory');
...
function appToSourceFolder(mapping, existingFolders) {
...
async function runMailSync(token, driveId, options) {
  ...
  // The archive's own folder list, so a mapping that names a folder which no
  // longer exists does not resurrect it.
  let archiveFolders = [];
  try {
  ...
  const folderForApp = appToSourceFolder(opts.mapping, archiveFolders);
  ...
  const summary = {
    ...
    amountsWritten: [], amountsUpdated: [], amountsNeedingReview: [], amountsUnread: [], amountsSkippedFilled: [], folderTotals: [], duplicateInvoices: [], looseIncluded: [],
  };
  const indexEntries = [];
  const marks = [];
