  ...
  for (const { name, key } of entries) {
    ...
    if (!entry) {
      ...
      try {
        ...
        const { text, pages, error, reader } = await readPdfText(bytes);
        entry = error ? { amount: null, currency: null, usable: false, note: error } : extractInvoiceTotal(text, { ...(budget.options || {}), pages });
  ...
  for (const message of messages) {
    ...
    for (const att of pdfs) {
      ...
      try {
        ...
        const { text: pdfText, pages: pdfPages, error: pdfError } = await readPdfText(bytes);
        ...
        const total = pdfError
          ...
          : extractInvoiceTotal(pdfText, { currency: verdict.currency, pages: pdfPages });
...
async function rereadHeld(token, driveId, ctx) {
  ...
  for (const item of candidates) {
    ...
    try {
      ...
      const total = extractInvoiceTotal(text, { currency: verdict.currency, pages });
