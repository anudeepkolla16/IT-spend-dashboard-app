...
const { extractInvoiceTotal } = require('./invoice-amount');
...
function planAmountCells(amounts, grid, used, cellValueOf) {
  ...
  for (const [app, byMonth] of Object.entries(amounts || {})) {
    ...
    for (const [month, amount] of Object.entries(byMonth)) {
      ...
      if (current !== null && current !== 0) {
        skippedFilled.push({ app, month, address, current, invoiceTotal: amount });
        continue;
      }
      write.push({ app, month, address, value: amount });
    }
  }
  return { write, skippedFilled };
}
...
async function runMailSync(token, driveId, options) {
  ...
  const summary = {
    ...
    base, newFolders: [],
    amountsWritten: [], amountsNeedingReview: [], amountsUnread: [], amountsSkippedFilled: [],
  };
  const indexEntries = [];
  const marks = [];
  ...
  for (const message of messages) {
    ...
    for (const att of pdfs) {
      ...
      const alreadyFiled = existing.has(fileName);
      try {
        ...
        // Read the payable total. Anything not plainly USD is recorded but never
        // totalled towards the sheet — see lib/invoice-amount.js.
        let total = { amount: null, currency: null, usable: false, note: 'not read' };
        try {
          const parsed = await pdfParse(bytes);
          total = extractInvoiceTotal(parsed && parsed.text);
        } catch (e) {
          total = { amount: null, currency: null, usable: false, note: `Could not read the PDF (${e.message}).` };
        }

        if (app && month && total.usable) {
          const byMonth = amounts[app] || (amounts[app] = {});
          byMonth[month] = Math.round(((byMonth[month] || 0) + total.amount) * 100) / 100;
        } else if (app && total.amount !== null && !total.usable) {
          summary.amountsNeedingReview.push({ app, month, file: fileName, amount: total.amount, currency: total.currency, note: total.note });
        } else if (app && total.amount === null) {
          summary.amountsUnread.push({ app, month, file: fileName, note: total.note });
        }
    ...
    try {
      ...
      if (hasAmounts) {
        try {
          ...
          const planned = planAmountCells(amounts, live.grid, { values: live.values, start: live.start }, cellValue);
          summary.amountsSkippedFilled = planned.skippedFilled;
          if (planned.write.length) {
            const results = await excel.writeCells(token, live.driveId, live.itemId, live.sheetName, planned.write, sessionId, 4);
            summary.amountsWritten = results.filter(r => r.ok).map(r => ({ app: r.app, month: r.month, address: r.address, amount: r.value }));
            for (const bad of results.filter(r => !r.ok)) summary.errors.push(`${bad.app} ${bad.month}: ${bad.error}`);
            if (summary.amountsWritten.length) {
              await appendLog(token, driveId, {
                ...
                cells: summary.amountsWritten.map(c => ({ app: c.app, month: c.month, address: c.address, before: null, after: c.amount })),
                failed: [],
              });
            }
          }
...
module.exports = { runMailSync, appToSourceFolder, monthFolderName, sourceBase, planTrackerCells, planAmountCells, STATE_PATH, INDEX_PATH };
