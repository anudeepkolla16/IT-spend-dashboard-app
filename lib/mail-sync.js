  ...
  for (const { name, entry } of items) {
    ...
    if (Array.isArray(entry.parts) && entry.parts.length) {
      ...
      for (const part of entry.parts) {
        if (part.ref && seenRefs.has(part.ref)) { duplicates.push({ file: name, amount: part.amount, ref: part.ref, of: seenRefs.get(part.ref), within: true }); continue; }
        if (part.ref) seenRefs.set(part.ref, name);
        sum += part.amount; fresh++;
      }
      if (fresh) { total += sum; counted++; }
