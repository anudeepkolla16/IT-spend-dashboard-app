            ...
            for (const r of ok) {
              ...
              if (u) {
                summary.amountsUpdated.push({
                  ...
                  previous: u.previous, direction: u.direction, emptied: !!u.emptied,
...
function formatRunReport(summary, heldItems, extra) {
  ...
  if (wrote.length || upd.length) {
    ...
    for (const u of upd) lines.push(`• ${u.app} ${u.month}: ${money(u.previous)} → ${money(u.amount)}${u.emptied ? ' (its invoices moved to another month)' : u.direction === 'down' ? ' (lowered — are all that month\'s invoices in the folder?)' : ''}${u.wasOurs ? '' : ' (replaced a figure not from invoices)'}`);
  }
...
module.exports = { runMailSync, resolvePending, collectSlackAnswers, notify, formatRunReport, readRules, writeRules, monthForInvoice, companionsOf, appToSourceFolder, reconcileArchive, recentMonths, RECONCILE_MONTHS, monthFolderName, planTrackerCells, planAmountCells, folderChildren, sumFolderInvoices };
