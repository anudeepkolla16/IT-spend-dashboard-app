...
async function sumFolderInvoices(token, driveId, folderPath, cache, budget) {
  ...
  return {
    ...
    pdfCount: pdfNames.length - duplicates.length,
...
async function runMailSync(token, driveId, options) {
  ...
  const summary = {
    ...
    amountsWritten: [], amountsUpdated: [], amountsNeedingReview: [], amountsUnread: [], amountsSkippedFilled: [], folderTotals: [], duplicateInvoices: [],
  };
  const indexEntries = [];
  const marks = [];
