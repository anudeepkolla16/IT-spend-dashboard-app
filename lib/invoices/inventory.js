const { listFilesRecursive, readJsonFile, resolveArchiveRoot, archiveFile } = require('../graph');
...
async function appFolders(token, driveId, rootId, rootPath) {
  ...
  if (!res.ok) throw new Error(`Graph listing of "${rootPath}" failed (${res.status})`);
