    ...
    while (cursor < folders.length) {
      ...
      try {
        ...
        for (const f of found) {
          ...
          const month = byFolder || monthFromFileName(f.name, f.createdDateTime, dayFirst);
          files.push({
            app: folder.name,
            month,
            monthSource: byFolder ? 'folder' : (month ? 'filename' : null),
            name: f.name,
            subfolder: f.relPath || '',
            size: f.size || 0,
...
module.exports = {
  ...
  amountsByFileName, monthIndexFromWord, isPlausibleMonth,
};
