const { encodeGraphPath, listFilesRecursive, readJsonFile, graphFetch, graphListAll } = require('../graph');
...
async function appFolders(token, driveId, rootId) {
  ...
  const children = await graphListAll(token, url, `Graph listing of "${ROOT}"`);
