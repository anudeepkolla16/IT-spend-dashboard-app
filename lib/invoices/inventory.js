const { encodeGraphPath, listFilesRecursive, readJsonFile } = require('../graph');
...
//
// The archive read here is Invoices/{App}/{Month}/, which is the mirrored,
// dashboard-facing copy the folder sync maintains — not the procurement folder
// it mirrors from. Reading the mirror keeps this to one drive and matches what
// the per-app invoice list in the app modal already shows.
...
async function folderId(token, driveId, path) {
  const url = `https://graph.microsoft.com/v1.0/drives/${encodeURIComponent(driveId)}/root:/${encodeGraphPath(path)}?$select=id`;
  const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
  if (res.status === 404) return null;
  if (!res.ok) throw new Error(`Graph folder lookup failed (${res.status}) for "${path}"`);
  return (await res.json()).id;
}

async function appFolders(token, driveId, rootId) {
  ...
  const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
  if (!res.ok) throw new Error(`Graph listing of "${ROOT}" failed (${res.status})`);
  const json = await res.json();
  // The sync's own bookkeeping files (_invoice-index.json, _sync-config.json)
  // and its unmatched bucket are not applications.
  return (json.value || []).filter(c => c.folder && !String(c.name).startsWith('_'));
}
...
async function buildInventory(token, driveId, opts) {
  ...
  const rootId = await folderId(token, driveId, ROOT);
  if (!rootId) return { scannedAt: new Date().toISOString(), files: [], apps: [], months: [], truncated: false, empty: true };

  const folders = await appFolders(token, driveId, rootId);
  const index = await readJsonFile(token, driveId, INDEX_PATH).catch(() => null);
...
module.exports = { buildInventory, parseMonthFolder, monthFromRelPath, amountsByFileName, monthIndexFromWord, INDEX_PATH, ROOT };
