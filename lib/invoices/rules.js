...
function classify(rules, signals, text, appNames) {
  ...
  if (vendor.app) {
    ...
    if (app) return { app, vendor: vendor.name, via, confident: true, currency: vendor.currency || null };
    return {
  ...
  for (const c of candidates) {
    ...
    if (app) return { app, vendor: vendor.name, via: `${via}+text`, matched: phrase, confident: true, currency: vendor.currency || null };
  }
  const options = candidates.map(c => resolveToSheetApp(c.app, appNames) || (Array.isArray(c.app) ? c.app[0] : c.app));
  return {
    app: null, vendor: vendor.name, reason: 'no-line-item-rule',
...
module.exports = { RULES_FILE, SEED_RULES, normalizeRules, matchVendor, classify, learn, accountPrefix, phraseIn, domainOf };
