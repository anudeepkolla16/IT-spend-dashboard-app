...
const { previousMonth } = require('../invoice-period');
...
// Each lock carries the seed round it arrived in (`since`); a live file
// records the last round applied as `locksSeeded`, so a later round can add
// its locks without touching anything the owner has since changed or removed.
//   round 1 — the five cells the owner named on 3 Sep 2026
//   round 2 — Cumul(Luzmo) Aug 2026, a statement figure the run of 3 Sep
//             lowered to one invoice's 557.28; the archive is short, as with
//             its July, and the sheet's figure is the right one
const SEED_LOCKS = [
  { since: 2, app: 'Cumul(Luzmo)', month: '2026-08', value: 14638, note: 'Restored on 3 Sep 2026: the sync lowered this to the one invoice on file (557.28); the archive is short and the sheet figure stands.' },
...
function upgradeRules(raw) {
  ...
  const round = l => l.since || 1;
  const latest = Math.max(...SEED_LOCKS.map(round));
  if (!Object.prototype.hasOwnProperty.call(live, 'locks')) {
    live.locks = SEED_LOCKS.map(l => { const { since, ...rest } = l; return rest; });
    live.locksSeeded = latest;
    changed = true;
  } else {
    const applied = Number(live.locksSeeded) || 1;
    if (applied < latest) {
      const locks = Array.isArray(live.locks) ? live.locks : (live.locks = []);
      const key = l => `${String(l.app || '').toLowerCase().replace(/[^a-z0-9]/g, '')}||${l.month}`;
      for (const seed of SEED_LOCKS.filter(l => round(l) > applied)) {
        if (!locks.some(l => l && key(l) === key(seed))) { const { since, ...rest } = seed; locks.push(rest); changed = true; }
      }
      live.locksSeeded = latest;
      changed = true;
    }
...
function normalizeRules(raw) {
  ...
  for (const v of list) {
    ...
    // "usage": the vendor's line-item ranges are its billing period (every
    // vendor's are read now; the value is kept for rules that still say it).
    // "advance" / "arrears": how the vendor bills when an invoice states no
    // period at all — the month it is dated, or the month before. Learned from
    // the owner's first answer for the vendor, or set by hand in the rules.
    if (v.period && /^(usage|advance|arrears)$/i.test(String(v.period))) out.period = String(v.period).toLowerCase();
  ...
  return { version: 1, vendors, locks, locksSeeded: Number(src.locksSeeded) || undefined };
}
...
// A vendor's billing convention for invoices that state no period, or null
// when it is not known yet ("usage" says nothing about that case).
const conventionOf = (period) => (period === 'advance' || period === 'arrears' ? period : null);

// The convention of whichever vendor files into this app's row.
function conventionFor(rules, app) {
  const key = norm(app);
  if (!key) return null;
  for (const v of normalizeRules(rules).vendors) {
    const rows = [].concat(v.app || [], (v.apps || []).map(a => a.app));
    if (rows.some(a => resolveKey(a) === key)) return conventionOf(v.period);
  }
  return null;
}

// The owner's answer to "which month?" for an invoice that stated no period
// says how the vendor bills: the month the invoice is dated (in advance) or the
// month before it (in arrears). Remembered on the vendor's rule, so every later
// invoice of its kind files itself and the vendor is asked once.
function learnPeriod(rules, item, month) {
  const out = normalizeRules(rules);
  if (!item || !month || item.periodStart || !item.invoiceDate) return { rules: out, learned: null };
  const vendor = out.vendors.find(v => v.name === item.vendor);
  if (!vendor) return { rules: out, learned: null };
  const issuedMonth = String(item.invoiceDate).slice(0, 7);
  let convention = null;
  if (month === issuedMonth) convention = 'advance';
  else if (month === previousMonth(issuedMonth)) convention = 'arrears';
  if (!convention || vendor.period === convention) return { rules: out, learned: null };
  vendor.period = convention;
  return {
    rules: out,
    learned: convention === 'advance'
      ? `${vendor.name} bills the month ahead: an invoice stating no period is the month it is dated`
      : `${vendor.name} bills in arrears: an invoice stating no period is the month before its date`,
  };
}
...
module.exports = { RULES_FILE, SEED_RULES, SEED_LOCKS, upgradeRules, normalizeRules, lockFor, matchVendor, classify, learn, learnPeriod, conventionOf, conventionFor, accountPrefix, phraseIn, domainOf };
