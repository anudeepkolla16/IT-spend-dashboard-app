...
const { openSpendSheet } = require('../spend-sheet');
...
async function planBackfill(token, marks, sessionId) {
  ...
  const sheet = await openSpendSheet(token, sessionId);
  const tracker = await openTracker(token, sheet.driveId, sheet.itemId, sessionId);
  if (!tracker) {
    return { sheet: null, requested: wanted.length, cells: [], alreadyTicked: [], offGrid: wanted };
  }
  ...
  return { sheet: tracker.name, requested: wanted.length, cells, alreadyTicked, offGrid };
}
...
async function applyBackfill(token, marks, sessionId) {
  const wanted = normalizeMarks(marks);
  const sheet = await openSpendSheet(token, sessionId);
  const result = await markTracker(token, sheet.driveId, sheet.itemId, wanted, sessionId);
  return { ...result, requested: wanted.length };
}

module.exports = { planTrackerCells, trackerRowIndex, openTracker, markTracker, planBackfill, applyBackfill, normalizeMarks, MAX_MARKS };
