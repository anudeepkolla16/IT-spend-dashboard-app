...
const { readJsonFile, writeJsonFile, uploadFileContent, archiveFile } = graph;
...
// Records a question about a cell — the invoices on file come to less than
// the sheet holds — with nothing to park. One per app-month.
function holdCell(state, meta) {
  const existing = state.items.find(i => i.kind === 'cell' && i.app === meta.app && i.month === meta.month);
  if (existing) { Object.assign(existing, { sheetValue: meta.sheetValue, invoiceTotal: meta.invoiceTotal, question: meta.question }); return { item: existing, already: true }; }
  const id = `P${state.nextId++}`;
  const item = { id, kind: 'cell', createdAt: new Date().toISOString(), file: `${meta.app} ${meta.month}`, options: ['keep the sheet figure', 'use the invoice total'], ...meta };
...
// Moves the held PDF out of _Pending. `options.rename` lets it take a numbered
// name when the destination already holds one of that name (_Ignored fills
// with repeats); `options.tolerateMissing` returns null instead of throwing
// when the held file is already gone — for ignoring, a file that is not there
// is not a problem, and the question should still close.
async function moveHeld(token, driveId, item, destFolderPath, options) {
  const opts = options || {};
  // Called through the module so a test can stand in for Graph.
  const itemId = await graph.itemIdByPath(token, driveId, item.heldPath);
  if (!itemId) {
    if (opts.tolerateMissing) return null;
    throw new Error(`the held file is no longer at "${item.heldPath}"`);
  }
  const parentId = await graph.ensureFolder(token, driveId, destFolderPath);
  const moved = await graph.moveItem(token, driveId, itemId, parentId, item.file, { rename: !!opts.rename });
  return `${destFolderPath}/${(moved && moved.name) || item.file}`;
}
...
// "all = ignore" closes every open question at once — the dashboard's
// "Ignore all" button, from Slack. Only ignore: an app or a month for every
// held file at once is never right.
const ALL_IGNORE = /^\s*(?:all|everything|all of them)\s*(?:=|:|->|→|-|–|—)?\s*(?:ignore|skip|drop)\s*$/i;

function parseReply(text, items, appNames, now) {
  const out = [];
  for (const line of String(text || '').split(/\r?\n/)) {
    if (ALL_IGNORE.test(line)) {
      for (const item of items) out.push({ id: item.id, ignore: true, line });
      continue;
    }
    ...
    // A question about a cell, not a file: "keep" holds (and locks) the sheet
    // figure, "invoices" writes the invoice total, a number sets and locks it.
    if (item.kind === 'cell') {
      if (/^(1|keep|sheet|keep the sheet( figure)?|lock)$/i.test(answer)) { out.push({ id, keep: true, line }); continue; }
      if (/^(2|invoices?|use( the)? invoices?( total)?|total)$/i.test(answer)) { out.push({ id, useInvoices: true, line }); continue; }
      const n = Number(answer.replace(/[$,\s]/g, ''));
      if (Number.isFinite(n) && n >= 0) { out.push({ id, value: n, line }); continue; }
      out.push({ id, error: `${id} is about a cell: reply keep, invoices, or a figure`, line });
      continue;
    }
...
function describeQuestions(items) {
  ...
  for (const it of items.slice(0, 20)) {
    if (it.kind === 'cell') {
      ...
      lines.push(`• *${it.id}* — ${it.app} ${it.month}: the sheet has ${money(it.sheetValue)}, the invoices on file come to ${money(it.invoiceTotal)}`);
      lines.push(`   Reply \`${it.id} = keep\` to keep (and lock) the sheet figure, \`${it.id} = invoices\` to write ${money(it.invoiceTotal)}, or \`${it.id} = <amount>\` to set and lock a figure.`);
      continue;
    }
  ...
  lines.push('');
  lines.push('Reply one per line, e.g. `P12 = Google Voice` or `P12 = 2` (option number), add the month if asked: `P12 = Google Voice, Aug-26`. `P12 = ignore` leaves it out of the sheet; `all = ignore` closes every open question.');
  return lines.join('\n');
}

module.exports = { PENDING_FILE, HOLD_FOLDER, readPending, writePending, holdInvoice, holdCell, findHeld, removeItem, moveHeld, parseReply, matchApp, describeQuestions };
