...
async function scanPeriods(token, driveId, opts) {
  ...
  if (summary.moves.length) {
    try {
      summary.cells = sheet ? predictCells(sheet, folders, movedOut, movedIn, appOf, index.written) : [];
    } catch (e) {
...
//
// So a cell is rewritten only when the invoices are demonstrably what it holds:
// it is empty, or it matches the folder's total before the moves, or it matches
// what this app last wrote there. Anything else is reported with both figures
// and left for a human, which is the same standard the invoice sync applies when
// it declines to overwrite a cell it did not write.
function decideCell({ current, before, after, ourLastWrite }) {
  ...
  return {
    write: false,
    blocked: `the sheet holds ${current.toLocaleString('en-US', { minimumFractionDigits: 2 })}, which is not what the invoices in that month come to`
      + `${before === 0 ? '' : ` (${before.toLocaleString('en-US', { minimumFractionDigits: 2 })} before this change)`}`
      + ` — the invoice moved, the cell is yours to adjust`,
  };
}
...
function predictCells(sheet, folders, movedOut, movedIn, appOf, priorWrites) {
  ...
  for (const [key, state] of after) {
    ...
    if (!verdict.write) {
      if (verdict.blocked) {
        // "the invoices come to 585.15" does not say whether the archive is
        // short or the sheet is stale, and those want opposite answers: Luzmo's
        // July is three invoices totalling 1,727.58 against a 13,439.00 charge
        // (the archive is missing some), while Shopify's February is one
        // invoice for 19.00 against 20.00 (the sheet is a little off). Naming
        // them turns a judgement call into a glance.
        cells.push({
          app: state.app, month: state.month, current, would: total, blocked: verdict.blocked,
          invoices: state.files.map(f => ({ file: f.name, amount: f.usable ? f.amount : null })),
        });
      }
      continue;
    }
    ...
    cells.push({
      ...
      current,
      // Carried to the apply, which re-derives the figure but must agree that
      // this cell was the invoices' to rewrite in the first place.
      before: state.before,
      value: total,
...
async function writeCells(token, driveId, totals, summary) {
  ...
  try {
    ...
    for (const t of totals) {
      ...
      if (!verdict.write) {
        if (verdict.blocked) summary.cellsSkipped.push({ app: t.app, month: t.month, why: verdict.blocked });
        continue;
      }
