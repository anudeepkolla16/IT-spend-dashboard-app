...
const { readInvoiceTotal } = require('../invoice-amount');
const { extractBillingPeriod, extractInvoiceDate, monthForPeriod, previousMonth } = require('../invoice-period');
const { conventionFor } = require('./rules');
...
// Bumped when the period reader learns a new form. A cached read that found no
// period under an older reader is read again, since the period may be there
// after all (line-item ranges, from version 2); one that found a period stands.
const PERIOD_VERSION = 2;
const LIST_POOL = 6;
const READ_POOL = 6;
...
function cachedReads(index) {
  ...
  for (const e of (Array.isArray(index && index.periods) ? index.periods : [])) {
    if (e && e.path && (e.pv === PERIOD_VERSION || e.periodStart)) periods.set(e.path, e);
  }
  return periods;
}
...
async function readInvoice(token, driveId, path) {
  ...
  const { text, error, total } = await readInvoiceTotal(bytes);
  if (error) return { path, read: false, note: error, pv: PERIOD_VERSION };
...
function planMove(file, folder, base) {
  ...
  // it states one, otherwise its date read the way this vendor bills — the
  // month it is dated, or the month before. The date alone is not enough: an
  // invoice of 1 September is August's from a vendor billing in arrears.
  if (!file.folderMonth && !file.nameMonth) {
    const byPeriod = file.periodStart
      ...
      : null;
    const convention = folder.convention || null;
    const byDate = !byPeriod && file.invoiceMonth && convention
      ? (convention === 'arrears' ? previousMonth(file.invoiceMonth) : file.invoiceMonth)
      : null;
    const month = byPeriod || byDate || null;
    if (!month) {
      return {
        skip: file.invoiceMonth
          ? `is filed loose in "${folder.vendorFolder}" with no month in its name and no period stated inside, only its date (${file.invoiceDate}). Whether ${folder.app || folder.vendorFolder} bills the month before or the month ahead is not known yet — answer the mail sync's question for one of its invoices, or file this one by hand`
          : `is filed loose in "${folder.vendorFolder}" with no month in its name and none stated inside — file it under a month folder by hand`,
        kind: 'undated',
      };
    }
    ...
    return {
      move: {
        ...
        via: byPeriod ? 'period' : (convention === 'arrears' ? 'month before invoice date (billed in arrears)' : 'invoice-date (billed in advance)'),
        undated: true,
      },
    };
  }
  ...
  for (const { vendor, files } of listed) {
    ...
    for (const file of files) {
      ...
      if (!folders.has(bucket)) {
        const app = appOf(vendor.name);
        folders.set(bucket, {
          app,
          vendorFolder: vendor.name,
          // How this vendor bills when an invoice states no period, from its
          // rule — the mail sync learned it from the owner's first answer.
          convention: options.rules ? conventionFor(options.rules, app) : null,
  ...
  await Promise.all(Array.from({ length: Math.min(READ_POOL, pending.length) }, async () => {
    while (readCursor < pending.length) {
      ...
      try {
        ...
        entry = { path: target.path, read: false, note: e.message, pv: PERIOD_VERSION };
      }
...
module.exports = { skipEntry, SKIP_KINDS, PERIOD_VERSION, cachedReads, scanPeriods, applyBackfill, planMove, predictCells, decideCell, readInvoice };
