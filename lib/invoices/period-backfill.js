...
const {
  ...
  itemIdByPath, ensureFolder, moveItem,
} = require('../graph');
...
function planMove(file, folder, base) {
  ...
  // Nothing to compare against — the file is not in a month folder at all.
  if (!file.currentMonth) return null;
  ...
  return {
    move: {
      ...
      // The dashboard reads the mirror, so a move has to happen in both places
      // or the checklist keeps showing the invoice under the old month.
      mirrorFromPath: folder.app ? `${MIRROR_ROOT}/${folder.app}/${file.relPath}/${file.name}` : null,
      mirrorToFolderPath: folder.app ? `${MIRROR_ROOT}/${folder.app}/${toFolder}` : null,
...
async function scanPeriods(token, driveId, opts) {
  ...
  const appForFolder = new Map(Object.entries(folderForApp).map(([app, folder]) => [folder, app]));
  ...
  for (const vendor of vendorFolders) {
    ...
    for (const file of files) {
      ...
      const currentMonth = relPath ? parseMonthFolder(relPath.split('/')[0], file.createdDateTime) : null;
      ...
      if (!folders.has(bucket)) {
        folders.set(bucket, {
          app: appForFolder.get(vendor.name) || null,
      ...
      folders.get(bucket).files.push({ ...entry, name: file.name, path, relPath, id: file.id, currentMonth });
    }
  }
  ...
  if (summary.moves.length) {
    try {
      summary.cells = await predictCells(token, folders, movedOut, movedIn, appForFolder);
    } catch (e) {
...
async function predictCells(token, folders, movedOut, movedIn, appForFolder) {
  ...
  for (const [key, files] of movedIn) {
    ...
    if (!after.has(key)) after.set(key, { app: appForFolder.get(vendorFolder) || null, month, files: [] });
...
async function applyBackfill(token, driveId, payload, opts) {
  ...
  for (const move of moves) {
    ...
    try {
      ...
      // The dashboard reads the mirror, not the archive, so it has to follow.
      // A mirror copy that is not there yet is not an error: the folder sync
      // will copy the file from its new home on the next run.
      let mirrored = false;
      if (move.mirrorFromPath && move.mirrorToFolderPath) {
        try {
          const mirrorId = await itemIdByPath(token, driveId, move.mirrorFromPath);
          if (mirrorId) {
            const mirrorTarget = await ensureFolder(token, driveId, move.mirrorToFolderPath);
            await moveItem(token, driveId, mirrorId, mirrorTarget);
            mirrored = true;
          }
        } catch (e) {
          summary.errors.push(`${move.file}: archived, but the dashboard copy did not move (${e.message})`);
        }
      }

      summary.moved.push({ ...move, mirrored });
      if (move.app) {
  ...
  if (renames.length) await renameIndexPaths(token, driveId, renames);
...
async function renameIndexPaths(token, driveId, renames) {
  const index = (await readJsonFile(token, driveId, INDEX_PATH)) || {};
