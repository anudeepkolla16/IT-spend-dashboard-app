...
// bare dashes.
function skipEntry(file, folder, why) {
  return { file: file.name, folder: folder.vendorFolder, why };
}
...
function planMove(file, folder, base) {
  ...
  if (!file.folderMonth && !file.nameMonth) {
    ...
    if (!month) {
      return { skip: `is filed loose in "${folder.vendorFolder}" with no month in its name and none stated inside — file it under a month folder by hand` };
    }
...
async function scanPeriods(token, driveId, opts) {
  ...
  for (const folder of folders.values()) {
    for (const file of folder.files) {
      ...
      if (!verdict) continue;
      if (verdict.skip) { summary.skipped.push(skipEntry(file, folder, verdict.skip)); continue; }
...
function predictCells(sheet, folders, movedOut, movedIn, appOf, priorWrites) {
  ...
  for (const [key, state] of after) {
    ...
    if (!verdict.write) {
      if (verdict.blocked) cells.push({ app: state.app, month: state.month, current, would: total, blocked: verdict.blocked });
      continue;
    }
...
module.exports = { skipEntry, scanPeriods, applyBackfill, planMove, predictCells, decideCell, readInvoice };
