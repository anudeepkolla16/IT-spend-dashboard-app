...
// `options.rename`: when the destination already holds a file of that name,
// let Graph give this one a numbered name rather than refuse the move. Right
// for a parking folder like _Ignored, where the same invoice can be set aside
// twice; wrong for a month folder, where a second copy would count twice.
async function moveItem(token, driveId, itemId, newParentId, newName, options) {
  const body = { parentReference: { id: newParentId } };
  if (newName) body.name = newName;
  if (options && options.rename) body['@microsoft.graph.conflictBehavior'] = 'rename';
  const res = await graphFetch(
